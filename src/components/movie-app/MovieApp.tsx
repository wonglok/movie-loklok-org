...
export function MovieApp() {
  ...
  const { isSaving } = useAutoSave(
    folderHandle, hydrated, story, artStyle, customArtStyle, characters, scenes,
  );

  useLocalImages(
    folderHandle, hydrated, characters, scenes, updateCharacter, updateScene,
  );
  ...
  useEffect(() => {
    if (!folderHandle) { setHydrated(true); return; }
    (async () => {
      try {
        ...
        if (movieData) {
          ...
          if (movieData.artStyle && ART_STYLES.some((s) => s.key === movieData.artStyle))
  ...
  }, [folderHandle, setStory, setArtStyle, setCustomArtStyle, setCharacters, setScenes]);
  ...
  const handleChangeFolder = async () => {
    ...
    try {
      ...
      setPickerError(err instanceof Error ? err.message : "Failed to select folder");
    }
  };

  const handleExtractCharacters = async () => {
    if (!story.trim() || isGenerating || !apiKey) return;
    setError(null); setSavedPath(null); setExtracting(true);
    try {
      const extracted = await extractCharacters(story, apiKey);
      setCharacters(extracted.map((c) => ({ ...c, imageUrl: null, imageFilename: null })));
    } catch (err) {
  ...
  const handleGenerateCharacterImages = async () => {
    ...
    setError(null); setSavedPath(null); setGeneratingCharacters(true);
    try {
      ...
      for (let i = 0; i < updated.length; i++) {
        ...
        const localUrl = await saveAndLoadLocal(result.url, filename, characterDir);
        updated[i] = { ...char, imageUrl: localUrl, imageFilename: filename };
        await savePromptFile(result.prompt, `${id}.txt`, characterDir);
      }
      setCharacters(updated);
      setCharacterImages(updated.map((c) => c.imageUrl).filter(Boolean) as string[]);
      setSavedPath(`${folderName}/images/character`);
    } catch (err) {
      setError(err instanceof Error ? err.message : "Generation failed");
    } finally { setGeneratingCharacters(false); }
  };

  const handleRegenerateCharacter = async (index: number) => {
    const char = characters[index];
    if (!char || !apiKey) return;
    setError(null); setRegeneratingIndex(index);
    try {
  ...
  const handleGenerateSceneImages = async () => {
    ...
    try {
      ...
      for (let i = 0; i < updated.length; i++) {
        ...
        updated[i] = { ...scene, imageUrl: localUrl, imageFilename: filename };
        await savePromptFile(result.prompt, `${id}.txt`, sceneDir);
      }
      setScenes(updated);
      setSceneImages(updated.map((s) => s.imageUrl).filter(Boolean) as string[]);
  ...
  const handleRegenerateScene = async (index: number) => {
    ...
    setError(null); setSceneRegenIndex(index);
    try {
      const prompt = `Cinematic movie keyframe, ${effectiveStyle} animation style. Scene: ${scene.name}. ${scene.description}. Wide establishing shot, dramatic lighting, film composition.`;
      const result = await generateImage(prompt, apiKey);
      if (folderHandle) {
      ...
      setError(err instanceof Error ? err.message : "Regeneration failed");
    } finally { setSceneRegenIndex(null); }
  };

  const previewChar = previewType === "character" && previewIndex !== null
    ? characters[previewIndex] : null;
  const previewScene = previewType === "scene" && previewIndex !== null
    ? scenes[previewIndex] : null;
    ...
    <div className="h-full overflow-y-auto blender-scrollbar">
      <div className="max-w-5xl mx-auto px-6 py-12 flex flex-col gap-16">
        ...
        <section className="relative text-center">
          ...
          <h1 className="text-5xl font-bold text-white mb-4 tracking-tight">Movie Studio</h1>
          ...
          {isSaving && (
            <p className="text-neutral-500 text-xs mt-3 animate-pulse">Saving...</p>
          )}
        </section>
        ...
        {showSettings && (
          <SettingsModal
            ...
            onClose={() => { setShowSettings(false); setPickerError(null); }}
          />
        )}
        ...
        <section className="flex flex-col gap-4">
          ...
          <div className="grid grid-cols-3 gap-3">
            {ART_STYLES.map((style) => {
              const isSelected = !customArtStyle.trim() && artStyle === style.key;
              return (
                <button
                  key={style.key}
          ...
          <div className="flex flex-col gap-2">
            <label className="text-sm text-neutral-400">Or type a custom style:</label>
            <input
              type="text"
        ...
        {characters.length === 0 && (
          <section className="flex flex-col items-center gap-4">
            ...
            >
              {extracting ? (
                <><div className="animate-spin rounded-full h-5 w-5 border-2 border-black/20 border-t-black" /> Extracting Characters...</>
              ) : ("Extract Characters")}
            </button>
          </section>
        )}
        ...
        {characters.length > 0 && (
          <section className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-4">
              {characters.map((char, i) => (
                <CharacterCard
                  ...
                  onPreview={(idx) => { setPreviewIndex(idx); setPreviewType("character"); }}
            ...
            <div className="flex gap-2">
              <button
                onClick={() => setCharacters([...characters, { name: "", description: "", imageUrl: null, imageFilename: null }])}
              ...
              >
                {generatingCharacters ? (
                  <><div className="animate-spin rounded-full h-4 w-4 border-2 border-black/20 border-t-black" /> Generating...</>
        ...
        {error && (
          <section className="flex justify-center">
            <p className="text-red-400 text-sm bg-red-400/10 rounded-xl px-4 py-3 max-w-md text-center">{error}</p>
          </section>
        )}
        ...
        {characterImages.length > 0 && scenes.length === 0 && (
          <section className="flex flex-col items-center gap-4">
            ...
            >
              {extractingScenes ? (
                <><div className="animate-spin rounded-full h-5 w-5 border-2 border-black/20 border-t-black" /> Extracting Scenes...</>
              ) : ("Extract Scenes")}
            </button>
          </section>
        )}
            ...
            <div className="grid grid-cols-2 gap-4">
              {scenes.map((scene, i) => (
                <SceneCard
                  ...
                  onRemove={setRemoveIndex}
                  onPreview={(idx) => { setPreviewIndex(idx); setPreviewType("scene"); }}
                  folderHandle={folderHandle}
